...
type LogTables = {
  [key in LogType]: ILogTable;
}

const s3ServerAccessLog: ILogTable = {
  location: "s3://#{logBucketName}/#{prefix}#{logSrcResourceAccountId}/#{logSrcResourceRegion}/#{logSrcResourceId}",
...
const cloudFrontAccessLog: ILogTable = {
  location:
...
export interface ILogAnalyticsProps {
  athenaQueryOutputBucket?: S3Bucket;
}
...
export class LogAnalytics extends Construct {
  constructor(scope: Construct, id: string, props: ILogAnalyticsProps) {
    super(scope, id)
    ...
    if (!athenaQueryOutputBucket) return;
    ...
    new cdk.aws_athena.CfnWorkGroup(this, "WorkGroup", {
      ...
      workGroupConfiguration: {
        ...
        resultConfiguration: {
          outputLocation: athenaQueryOutputBucket.bucket.s3UrlForObject(),
        },
      },
    });
  }

  public createDatabase(id: string, props: ICreateDatabaseProps): cdk.aws_glue.CfnDatabase {
  ...
  public createTable(id: string, props: ICreateTableProps): void {
    const { locationPlaceholder, logType, databaseName } = props;
    if (locationPlaceholder.prefix) {
      locationPlaceholder.prefix = `${locationPlaceholder.prefix}/`;
    } else {
      locationPlaceholder.prefix = "";
    }

    const logTable = logTables[logType];
  ...
  private replacePlaceholders(template: string, props: Record<string, string>): string {
    ...
    for (const placeholder of placeholders) {
      if (props.hasOwnProperty(placeholder)) {
        ...
        throw new Error(`Failed to replace placeholder: #{${placeholder}}`)
      }
    }
